...
class wpdb {
	...
	public function set_charset( $dbh, $charset = null, $collate = null ) {
		...
		if ( $this->has_cap( 'collation' ) && ! empty( $charset ) ) {
			...
			if ( $this->use_mysqli ) {
				...
				if ( function_exists( 'mysqli_set_charset' ) && $this->has_cap( 'set_charset' ) ) {
					$set_charset_succeeded = mysqli_set_charset( $charset, $dbh );
				}
				if ( $set_charset_succeeded ) {
					$query = $this->prepare( 'SET NAMES %s', $charset );
					if ( ! empty( $collate ) ) {
	...
	/**
	 * Real escape, using mysqli_real_escape_string() or mysqli_real_escape_string()
	 *
	...
	function _real_escape( $string ) {
		if ( $this->dbh ) {
			if ( $this->use_mysqli ) {
				...
				$escaped = mysqli_real_escape_string( $string, $this->dbh );
			}
		} else {
	...
	public function query( $query ) {
		...
		if ( ! empty( $this->dbh ) ) {
			if ( $this->use_mysqli ) {
				if ( $this->dbh instanceof mysqli ) {
					$mysqli_errno = mysqli_errno( $this->dbh );
				} else {
		...
		if ( empty( $this->dbh ) || 2006 == $mysqli_errno ) {
				...
				if ( ! empty( $value['db'] ) ) {
					...
					if ( $this->charset ) {
						...
						if ( $this->use_mysqli ) {
							...
							$connection_charset = mysqli_client_encoding();
						}
					}
	...
	public function check_database_version() {
		global $wp_version, $required_mysqli_version;
		// Make sure the server has the required MySQL version
		if ( version_compare( $this->db_version(), $required_mysqli_version, '<' ) ) {
			/* translators: 1: WordPress version number, 2: Minimum required MySQL version number */
			return new WP_Error( 'database_version', sprintf( __( '<strong>ERROR</strong>: WordPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysqli_version ) );
		}
	}
